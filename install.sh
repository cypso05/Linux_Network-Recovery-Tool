...
# Version: 1.1.2
# Installs the tool, GUI wrapper, web interface, modular components, 
# XFCE integration, icons, polkit rules, and Python dependencies
#===============================================================================
...
if [[ "$UNINSTALL" == "true" ]]; then
    ...
    rm -f "$BIN_DIR/network-recover-dashboard" && echo "  ✅ Removed: $BIN_DIR/network-recover-dashboard" || echo "  ⚠️  Not found: $BIN_DIR/network-recover-dashboard"
...
if [[ "$SKIP_DEPS" != "true" ]]; then
    ...
    install_deps_debian() {
        apt-get update -qq
        local deps="curl iproute2 bridge-utils ethtool netcat-openbsd dnsutils network-manager policykit-1 python3 python3-pip"
        if [[ "$NO_GUI" != "true" ]]; then
    ...
    install_deps_rhel() {
        local deps="curl iproute bridge-utils ethtool nmap-ncat bind-utils NetworkManager polkit python3 python3-pip"
        if [[ "$NO_GUI" != "true" ]]; then
    ...
    install_deps_arch() {
        local deps="curl iproute2 bridge-utils ethtool openbsd-netcat bind networkmanager polkit python python-pip"
        if [[ "$NO_GUI" != "true" ]]; then
            deps="$deps zenity libnotify"
        fi
    ...
    case "$DISTRO" in
        ...
        *)
            ...
            echo "   core: curl, iproute2, bridge-utils, ethtool, netcat, dnsutils, network-manager, polkit, python3, python3-pip"
            if [[ "$NO_GUI" != "true" ]]; then
    ...
    # ------------------------------------------------------------------
    # STEP 1.5: Install Python dependencies for web server
    # ------------------------------------------------------------------
    echo ""
    ...
    # --- psutil ---
    if py_module_available "psutil"; then
        ...
        case "$DISTRO" in
            debian|ubuntu|mx|linuxmint|pop)
                apt-get install -y -qq python3-psutil 2>/dev/null && PSUTIL_INSTALLED=true
                ;;
            fedora|rhel|centos|rocky|alma)
                dnf install -y python3-psutil 2>/dev/null && PSUTIL_INSTALLED=true
                ;;
            arch|manjaro|endeavouros)
                pacman -S --noconfirm python-psutil 2>/dev/null && PSUTIL_INSTALLED=true
                ;;
        esac
        
        # Fallback to pip
        if [[ "$PSUTIL_INSTALLED" != "true" ]]; then
            pip3 install psutil 2>/dev/null && PSUTIL_INSTALLED=true
        fi
        
        if py_module_available "psutil"; then
            echo "  ✅ psutil installed successfully"
        else
            echo "  ⚠️  psutil installation failed - VM metrics will be limited"
            PYTHON_DEPS_MISSING=true
        fi
    fi

    # --- websockets ---
    if py_module_available "websockets"; then
        ...
        case "$DISTRO" in
            debian|ubuntu|mx|linuxmint|pop)
                apt-get install -y -qq python3-websockets 2>/dev/null && WEBSOCKETS_INSTALLED=true
                ;;
            fedora|rhel|centos|rocky|alma)
                dnf install -y python3-websockets 2>/dev/null && WEBSOCKETS_INSTALLED=true
                ;;
            arch|manjaro|endeavouros)
                pacman -S --noconfirm python-websockets 2>/dev/null && WEBSOCKETS_INSTALLED=true
                ;;
        esac
        
        # Fallback to pip
        if [[ "$WEBSOCKETS_INSTALLED" != "true" ]]; then
            pip3 install websockets 2>/dev/null && WEBSOCKETS_INSTALLED=true
        fi
        
        if py_module_available "websockets"; then
            echo "  ✅ websockets installed successfully"
        else
            echo "  ⚠️  websockets installation failed - real-time updates will be disabled"
            PYTHON_DEPS_MISSING=true
        fi
    fi
    ...
    if [[ "$PYTHON_DEPS_MISSING" == "true" ]]; then
        echo -e "${YELLOW}⚠️  Some Python dependencies are missing. Web server will run with reduced functionality.${NC}"
        echo "   Missing modules can be installed manually:"
        echo "     sudo pip3 install psutil websockets"
    else
...
#===============================================================================
# STEP 4.5: Install dashboard script
#===============================================================================
echo ""
echo "📊 Installing dashboard script..."
if [[ -f "$SCRIPT_DIR/src/network-recover-dashboard" ]]; then
    cp "$SCRIPT_DIR/src/network-recover-dashboard" "$BIN_DIR/network-recover-dashboard"
    chmod 755 "$BIN_DIR/network-recover-dashboard"
    echo -e "${GREEN}✅ Dashboard script installed to $BIN_DIR/network-recover-dashboard${NC}"
else
...
#===============================================================================
# STEP 5.7: Remove BOM from scripts
#===============================================================================
echo ""
echo "🔧 Removing BOM from scripts..."
for file in "$BIN_DIR/network-recover" "$BIN_DIR/network-recover-gui" "$BIN_DIR/network-recover-web" "$BIN_DIR/network-recover-dashboard"; do
    if [[ -f "$file" ]]; then
...
#===============================================================================
# STEP 9.5: Refresh XFCE panel cache
#===============================================================================
echo ""
...
# Dashboard script
if [[ -x "$BIN_DIR/network-recover-dashboard" ]]; then
    echo -e "  ✅ Dashboard: $BIN_DIR/network-recover-dashboard"
else
    echo -e "  ⚠️  Dashboard: missing (optional)"
fi
...
if python3 -c "import psutil" 2>/dev/null; then
    echo -e "  ✅ psutil available"
else
    echo -e "  ⚠️  psutil NOT available (pip3 install psutil)"
fi
if python3 -c "import websockets" 2>/dev/null; then
    echo -e "  ✅ websockets available"
else
    echo -e "  ⚠️  websockets NOT available (pip3 install websockets)"
fi
...
if [[ "$VERIFY_OK" == "true" ]]; then
    ...
    echo "    network-recover-dashboard"
    echo ""
    echo "  Web Interface:"
